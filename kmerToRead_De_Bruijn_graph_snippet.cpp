...
int main(int argc, char ** argv){
	...
	while ((ch = getopt (argc, argv, "g:k:m:n:s:t:b:e:o:")) != -1){
		switch(ch){
			//~ case 'q':
				//~ query=optarg;
				//~ break;
	...
	}
	{
		...
		vector<uint8_t> v;
		vector<vector<uint8_t>> kmerToRead(ksl.number_kmer,v);
				...
				while(not in.eof()){
					...
					for(uint i(0);i<100;++i){
						...
						if(line[0]=='A' or line[0]=='C' or line[0]=='G' or line[0]=='T'){
							...
							for(uint64_t j(0);j<kmer_ids.size();++j){
								//I add a new read index
								if(kmer_ids[i]>=0){
									#pragma omp critical(color)
									{
										kmerToRead[kmer_ids[j]].push_back(i);
									}
								}
							}
						}
				...
				//~ for(i=(0);i<lines.size();++i){
					...
					//~ if(line[0]=='A' or line[0]=='C' or line[0]=='G' or line[0]=='T'){
						...
						//~ for(uint64_t i(0);i<kmer_ids.size();++i){
							...
							//~ if(kmer_ids[i]>=0){
							//~ // write the read ids corresponding to the kmer
								//~ for (auto && read: kmerToRead[kmer_ids[i]]){
									//~ toWrite+=to_string(read) + " ";
								//~ }
